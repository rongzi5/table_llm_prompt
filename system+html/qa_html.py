...
def run_qa_evaluation(
    ...
    for idx, item in enumerate(random_data_list):
        ...
        user_prompt = "table_text:" + html + "; statement: " + item.get("question", "")
        # 构建聊天会话
        if len(user_prompt) > 32768:
            print(f"Warning: Instruction too long ({len(user_prompt)} tokens), truncating.")
            user_prompt = user_prompt[:32768]  # 截断为最大长度
